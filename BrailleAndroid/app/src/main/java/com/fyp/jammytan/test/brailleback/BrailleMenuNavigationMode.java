...
import com.fyp.jammytan.test.labeling.CustomLabelManager;
import com.fyp.jammytan.test.labeling.Label;
import com.fyp.jammytan.test.labeling.LabelOperationUtils;
import com.fyp.jammytan.test.utils.AccessibilityNodeInfoRef;
import com.fyp.jammytan.test.utils.AccessibilityNodeInfoUtils;
 ...
 * Braille display. Not to be confused with navigation through normal on screen
 * options menu items.
 * This is a menu that is shown only on the Braille display for unlabeled items
 * for adding, editing, or removing labels.
 */
public class BrailleMenuNavigationMode implements NavigationMode {
    ...
    public boolean onPanLeftOverflow(DisplayManager.Content content) {
        ...
        return mFeedbackManager.emitOnFailure(
            scrollMenu(SCROLL_MENU_BACKWARD),
    ...
    public boolean onPanRightOverflow(DisplayManager.Content content) {
        ...
        return mFeedbackManager.emitOnFailure(
            scrollMenu(SCROLL_MENU_FORWARD),
            FeedbackManager.TYPE_NAVIGATE_OUT_OF_BOUNDS);
    }
    ...
    private boolean launchIntentToPlayStoreTalkBack() {
        try {
            mAccessibilityService.startActivity(new Intent(Intent.ACTION_VIEW,
                    Uri.parse(TALKBACK_URL_PLAYSTORE))
                    .addFlags(Intent.FLAG_ACTIVITY_NEW_TASK));
        } catch (ActivityNotFoundException e) {
            mAccessibilityService.startActivity(new Intent(Intent.ACTION_VIEW,
                    Uri.parse(TALKBACK_URL_WEB))
                    .addFlags(Intent.FLAG_ACTIVITY_NEW_TASK));
        }
        return true;
    }
    ...
    /**
     * Displays a menu for the node and populates {@code mMenuItems}.
     * If the node doesn't warrant a menu does nothing. Returns whether
     * a menu was shown.
     */
    ...
    /**
     * Calls the listener signaling that the menu has been closed and the
     * navigation mode should be exited.
     */
    ...
    /**
     * Scrolls the menu to the next item in the specified direction. Returns
     * whether the menu successfully scrolled.
     * {@code direction} should be either SCROLL_MENU_FORWARD
     * or SCROLL_MENU_BACKWARD.
     */
    ...
    /**
     * Updates the text on the braille display to match the currently selected
     * menu item.
     */
    private void brailleDisplayWithCurrentItem() {
        MenuItem current = mMenuItems.get(mCurrentIndex);
        DisplayManager.Content content =
                new DisplayManager.Content(current.getName());
