...
/**
 * Keeps track of the self-braille service and provides a means for querying
 * the self-brailled content.
 *
 * This implementation assumes that the {@link SelfBrailleService} is running
 * in the same process, and accesses a static reference to it.
 */
public class SelfBrailleManager {
    ...
    public DisplayManager.Content contentForNode(
        ...
        DisplayManager.Content content = service != null
                ? service.contentForNode(node)
                : null;
        if (content != null) {
    ...
    public boolean hasContentForNode(
            AccessibilityNodeInfoCompat node) {
    ...
    /**
     * Informs the self braille manager about the state of the IME input
     * view.  This is used to decide on whether to allow contracted braille
     * when generating the braille content.
     */
