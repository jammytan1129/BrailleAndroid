...
package com.fyp.jammytan.test.labeling;
...
import com.fyp.jammytan.test.labeling.AllLabelsFetchRequest.OnAllLabelsFetchedListener;
import com.fyp.jammytan.test.labeling.DirectLabelFetchRequest.OnLabelFetchedListener;
import com.fyp.jammytan.test.labeling.PackageLabelsFetchRequest.OnLabelsFetchedListener;
import com.fyp.jammytan.test.utils.AccessibilityEventListener;
import com.fyp.jammytan.test.utils.LogUtils;
import com.fyp.jammytan.test.utils.StringBuilderUtils;
 ...
 * of automatically prefetching labels into the managed cache based on the
 * {@link AccessibilityEvent}s delivered to the client application. In order for
 * this prefetching to occur, clients should send {@link AccessibilityEvent}s
 * through {@link #onAccessibilityEvent(AccessibilityEvent)}.
 *
...
public class CustomLabelManager implements AccessibilityEventListener {
    ...
    public static final String
            AUTHORITY = "com.google.android.marvin.talkback.providers.LabelProvider";
    ...
    private final Map<String, Map<String, Label>>
            mLabelCache = new LruCache<String, Map<String, Label>>(MAX_CACHE_SIZE);
     ...
     * @param callback The {@link OnLabelFetchedListener} to return the label
     *            though
     */
     ...
     * @param callback The {@link OnLabelsFetchedListener} to return the labels
     *            through
     */
     ...
     * {@link #getLabelForLabelIdFromDatabase(Long, OnLabelFetchedListener)} in
     ...
     *
     * @param packageNames specific package names to refresh, or {@code null}
     *            to refresh all existing. If a package name provided in this
     *            parameter does not exist in the cache, it will be prefetched
     */
    ...
    private void prefetchLabelsFromEvent(AccessibilityEvent event) {
        if (event.getEventType() == AccessibilityEvent.TYPE_WINDOW_CONTENT_CHANGED) {
            ...
            final LinkedList<AccessibilityNodeInfo> seenNodes = new LinkedList<
            ...
            while (!seenNodes.isEmpty()) {
                ...
                final Pair<String, String> resId = splitResourceName(
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent.hasExtra(LabelOperationUtils.EXTRA_STRING_ARRAY_PACKAGES)) {
                packages = intent.getStringArrayExtra(
                        LabelOperationUtils.EXTRA_STRING_ARRAY_PACKAGES);
            }
