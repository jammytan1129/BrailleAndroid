...
import com.fyp.jammytan.test.bluetooth.BluetoothProfileCompatHoneycomb.ServiceListenerBridge;
...
public class BluetoothProfileCompat {
    ...
    static class BluetoothProfileHoneycombImpl extends BluetoothProfileStubImpl {
        @Override
        public Object newServiceListener(final ServiceListenerCompat listener) {
            return BluetoothProfileCompatHoneycomb.newServiceListener(new ServiceListenerBridge() {
                @Override
    ...
    /* package */static final Class<?> CLASS_BluetoothProfile = CompatUtils
            .getClass("android.bluetooth.BluetoothProfile");

    private static final Method METHOD_getConnectedDevices = CompatUtils.getMethod(
            CLASS_BluetoothProfile, "getConnectedDevices");

    /**
     * Extra for the connection state intents of the individual profiles. This
     * extra represents the current connection state of the profile of the
     * Bluetooth device.
     */
    ...
    /**
     * Extra for the connection state intents of the individual profiles. This
     * extra represents the previous connection state of the profile of the
     * Bluetooth device.
     */
    ...
    public static List<BluetoothDeviceCompat> getConnectedDevices(Object receiver) {
        final List<?> results = (List<?>) CompatUtils.invoke(
                receiver, null, METHOD_getConnectedDevices);

        if (results == null) {
            return Collections.emptyList();
        }

        final List<BluetoothDeviceCompat> output = new ArrayList<BluetoothDeviceCompat>(results
                .size());
    ...
    public static abstract class ServiceListenerCompat {
        static final Class<?> CLASS_ServiceListenerCompat = CompatUtils
                .getClass("android.bluetooth.BluetoothProfile.ServiceListener");
