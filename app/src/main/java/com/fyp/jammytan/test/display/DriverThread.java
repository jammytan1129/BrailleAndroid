...
import com.fyp.jammytan.test.display.BrailleInputEvent;
 ...
 *
 * Threading model:
 * The methods of this class can be called from any thread.  Calls will be
 * forwarded to an internal thread and executed asynchronously.  The methods
 * of the nested callback interfaces will be invoked in an internal thread and
 * should therefore finish quickly without blocking.
 */
public class DriverThread {
    ...
    /**
     * Callback interface for getting notified when the driver initialization
     * has either succeeded or failed.
     */
    public interface OnInitListener {
        /**
         * Called when initialization either succeeded or failed.  In the
         * former case, {@code displayProperties} is non-null and contains the
         * properties of the connected display.  If initialization failed,
         * {@code displayProperties} is {@code null}.
         */
    ...
    public interface OnInputEventListener {
        /**
         * Dispatch an input event to interested users.  This is called
         * from the driver thread, so any lengthy processing should
         * be deferred to a different thread.
         */
...
//        try {
//            deviceInfo.getBluetoothDevice().createRfcommSocketToServiceRecord(deviceInfo.getSdpUuid()).connect();
//        }catch (IOException ex){}

        mBrlttyWrapper = new BrlttyWrapper(deviceInfo, this, resources, mOutputStream);
        mHandlerThread = new HandlerThread("DriverThread") {
            @Override
    ...
    /**
     * Add bytes that have been read from the device and wake up the
    ...
    public void stop() {
        ...
        try {
            ...
            if (mHandlerThread.isAlive()) {
                ...
                Log.e(LOG_TAG, "*** Driver thread takes very long to "
                        + "terminate -- giving up waiting ***");
            }
    ...
    private void writeWindowInternal() {
        ...
        if (buffer != null) {
            //mBrlttyWrapper.writeWindow(buffer);
            sendBytesToDevice(buffer);
        }
    }
     ...
     */
    public void sendBytesToDevice(byte[] bytes) {
        try {
...
//                        new BrailleInputEvent(command & COMMAND_CODE_MASK,
//                                ((command & COMMAND_ARGUMENT_MASK)
//                                 >> COMMAND_ARGUMENT_SHIFT),
